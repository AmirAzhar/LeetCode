...
var MedianFinder = function () {
  ...
  function addNum(num) {
    if (min.front() != null && num < min.front().element) max.enqueue(num);
    else min.enqueue(num);

    const diff = min.size() - max.size();

    if (diff > 1) max.enqueue(min.dequeue().element);

    if (diff < -1) min.enqueue(max.dequeue().element);
  }

  function findMedian() {
    const diff = min.size() - max.size();

    if (diff > 0) return min.front().element;
    else if (diff < 0) return max.front().element;
    else return (min.front().element + max.front().element) / 2;
  }
};
